...
def make_dirs(analysis_name):
    ...
    primary_dir = Path.joinpath(Path.cwd().parents[2], str(analysis_name))
    data_dir = Path.joinpath(Path.cwd().parents[2], str(analysis_name), "data")
    no_hits_dir = Path.joinpath(
        Path.cwd().parents[2], str(analysis_name), "data", "no_hits"
    )
    outputs_dir = Path.joinpath(Path.cwd().parents[2], str(analysis_name), "outputs")
